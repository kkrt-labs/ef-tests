...
# Runs the repo tests
tests:
	cargo nextest run --failure-output final --no-fail-fast --final-status-level fail

# Runs ef tests only
ef-test:
	cargo nextest run --package ef-testing --test tests --failure-output final --final-status-level fail --no-fail-fast
