...
lazy_static! {
    ...
    pub static ref FEE_TOKEN_CLASS_HASH: ClassHash = (*TryInto::<ContractClass>::try_into(FEE_TOKEN_CLASS.clone()).expect("Failed to convert token class hash").hinted_class_hash()).into();
...
pub mod kkrt_constants_v0 {
    ...
    lazy_static! {
        ...
        // Main class hashes
        pub static ref KAKAROT_CLASS_HASH: ClassHash = (*TryInto::<ContractClass>::try_into(KAKAROT_CLASS.clone()).expect("Failed to convert Kakarot class hash").hinted_class_hash()).into();
        pub static ref CONTRACT_ACCOUNT_CLASS_HASH: ClassHash = (*TryInto::<ContractClass>::try_into(CONTRACT_ACCOUNT_CLASS.clone()).expect("Failed to convert contract account class hash").hinted_class_hash()).into();
        pub static ref EOA_CLASS_HASH: ClassHash = (*TryInto::<ContractClass>::try_into(EOA_CLASS.clone()).expect("Failed to convert eoa class hash").hinted_class_hash()).into();
        pub static ref PROXY_CLASS_HASH: ClassHash = (*TryInto::<ContractClass>::try_into(PROXY_CLASS.clone()).expect("Failed to convert proxy class hash").hinted_class_hash()).into();
    }
}
